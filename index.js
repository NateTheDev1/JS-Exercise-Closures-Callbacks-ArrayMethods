...
function processDuplicateFree(list, callback) {
	return callback(list.filter((num, i) => list.indexOf(num) === i));
}
...
function getFullNames(runners) {
  let names = [];
	runners.forEach(r => {
    names.push(`${r.last_name}, ${r.first_name}`);
  });
  return names;
}
...
function tallyUpDonations(runners) {
	return runners.reduce((sum, currentValue) => sum + currentValue.donation, 0);
